...
> Версия 1.1, кандидат Open Path · 2026-09-23 · living source: `frontend/design-system.html`; визуальное утверждение владельца ожидается
...
|---|---|---|
| Ashyq Ink | `#111827` | фирменный примитив; продуктовый текст берёт семантический токен |
| Paper | `#F7F3EA` | фирменный примитив; продуктовый фон берёт семантический токен |
| Academic Blue | `#526DA6` | фирменный примитив; интерактивный цвет берёт семантический токен |
...
Кандидат Open Path использует отдельные семантические роли в рабочем UI: светлые фон `#F4F5F3`, текст `#243044`, действие `#3C5985`; тёмные фон `#202B35`, текст `#E9EEF0`, действие `#B0C7EB`. Источник истины для всех состояний и производных цветов — `frontend/src/styles/tokens.css`; компоненты используют только его алиасы второго слоя.
...
|---|---|---|
| Brand / action | Onest 600–800 | wordmark, кнопки, navigation, короткие продуктовые заголовки |
| Display | Onest 600–700 | H1 и начало раздела; иерархия задаётся размером и весом без отдельного декоративного шрифта |
| UI / body | Onest 400–700 | основной интерфейс, инструкции, формы, legal |
| User-facing data | Onest 400–700, tabular numerals | суммы, даты, оценки и другие значения, которые читает абитуриент |
| Technical identifiers | IBM Plex Mono 400–500 | коды, машинные идентификаторы и фрагменты исходных данных |
...
`OriginMark(fact) + readable value (tabular numerals when numeric) + EvidenceLink + FreshnessBadge`.
