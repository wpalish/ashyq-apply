...
Five primary destinations: Case, Shortlist, Plan, Community, More. Context navigation preserves all existing screens. The case overview chooses a next action from actual saved-profile, run and result state. Do not imply readiness from unvalidated draft fields or invent deadlines.

## 3. Anatomy

Desktop sidebar / mobile bottom navigation; horizontally scrollable contextual links on mobile; mode badge; case switcher; global running banner; main landmark. Account controls move into More on mobile. Overview: editorial greeting, next-action panel, three workflow steps, real result counts (unavailable summary explicitly says no data), source/data explanation.
...
Hydrating; no saved profile; saved profile/no run; queued/running; failed/cancelled; results ready. Failed or cancelled research offers progress/retry before results. Button focus/hover/disabled inherits the design system. No artificial success or progress percentages. Mobile navigation reserves bottom space and remains keyboard reachable.
