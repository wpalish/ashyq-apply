...
def _row(result: ProgramResult) -> dict[str, Any]:
    ...
    return {
        ...
        "ranking": "; ".join(f"{r.source} {r.year}: {r.position}" for r in result.rankings)
        or "not found",
        ...
        "scholarship_amount": (
            str(best.amount)
            if best and best.amount
            else (
                f"{best.amount_is_percentage_of_tuition:g}% of tuition"
                if best and best.amount_is_percentage_of_tuition
                else "not published"
            )
        ),
        "covered_costs": "; ".join(
        ...
        "funding_gap": (
            str(gap.gap)
            if gap and gap.computable and gap.gap
            else f"not computable — {gap.reason}"
            if gap
            else "not computed"
        ),
        ...
        or "not collected (approve to collect)",
        "conflicts": "; ".join(f"{c.subject}: {c.values}" for c in result.conflicts)
...
def to_xlsx(results: list[ProgramResult], meta: dict | None = None) -> bytes:
    ...
    for r in results:
        ...
        if fill:
            ws.cell(
                row=idx, column=[k for k, _ in COLUMNS].index("funding_classification") + 1
            ).fill = PatternFill("solid", fgColor=fill)

    widths = {
        "university": 30,
        "program": 34,
        "published_requirements": 46,
        "conflicts": 44,
        "source_links": 52,
        "post_study_work": 46,
        "covered_costs": 30,
